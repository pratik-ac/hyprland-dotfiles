...
hl.bind("switch:on:Lid Switch", hl.dsp.exec_cmd("/home/von/.config/hypr/scripts/lid-lock.sh"))

-- Trigger when the lid is opened: re-enable laptop screen
...
-- Rofi shortcuts
hl.bind(mainMod .. " + O", hl.dsp.exec_cmd("rofi -modi ssh,run -show ssh"))
hl.bind(mainMod .. " + C", hl.dsp.exec_cmd("rofi -show calc -modi calc -no-show-match -no-sort"))
hl.bind("ALT + C", hl.dsp.exec_cmd([[
    sh -c '
cliphist list | rofi -dmenu | cliphist decode | wl-copy && wtype -M ctrl shift -P v -m ctrl
'
]]))

-- GPU screen recorder script
hl.bind("ALT + Z", hl.dsp.exec_cmd("bash $HOME/.config/rofi/scripts/gpu-screen-recorder.sh"))
